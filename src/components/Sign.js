            ...
            <h2 className="sign__title">{title}</h2>
                <form className = "sign__form" id={`sign-${name}-form`} name={`sign-${name}-form`} onSubmit={handleSubmit}>
                    <fieldset className="sign__inputs">
                        ...
                        <input type="submit" className="sign__save-button" id={`sign-${name}__save-button`} value={buttonText} disabled={!(isValid.email && isValid.password && values.email && values.password)} style={children ? { marginBottom: '0'} : { marginBottom: '46px'}}/>
                    </fieldset>                
                </form>
            {children}
        </div>
